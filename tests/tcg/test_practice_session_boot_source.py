"""The Sam practice duel's boot/main-loop wiring: Memory's boot RAM zeroing,
PracticeSession's practice-deck load and turn-loop ordering, PlayerActions'
coverage of every scripted practice requirement, AI's native 7-turn Sam
script (including the original source bug), and Combat's fail-closed
effect dispatch.

(Split out of what was misnamed test_practice_playable_source.py -- that
name now belongs to the actual PracticePlayable.lua rendering tests; these
assertions are about the duel engine underneath it, not the presentation
layer, so they keep their own file.)
"""
...
class PracticeLoopBootSourceTests(unittest.TestCase):
